...
### Step (2): $\int_{a}^{b} \tilde{g}''(x)^2dx \ge \int_{a}^{b} \tilde{g}''(x)^2dx.$
...
\int_{a}^{b} \tilde{g}''(x)^2 dx &\ge \int_{a}^{b} \tilde{g}''(x)^2dx
\end{align*}
...
一方、$\tilde{g}$ は最適化問題の解であるので、$\int_{a}^{b} \tilde{g}''(x)^2 dx \le \int_{a}^{b} \tilde{g}''(x)^2dx$. 
よって、$\int_{a}^{b} \tilde{g}''(x)^2 dx = \int_{a}^{b} \tilde{g}''(x)^2dx$.
...
推定値が$\widehat{y} =  \mathbf{H} y$ と書けるタイプ（線形円滑化）のモデルについて、その自由度を
